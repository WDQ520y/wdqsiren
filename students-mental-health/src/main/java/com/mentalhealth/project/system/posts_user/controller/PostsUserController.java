...
public class PostsUserController extends BaseController
{
    ...
    public AjaxResult addSave(Posts posts)
    {
        ...
        // ===== 危机识别检测 =====
        String detectText = (posts.getPostsTitle() != null ? posts.getPostsTitle() : "")
                + " " + (posts.getPostsContent() != null ? posts.getPostsContent() : "");
        CrisisDetectionResult crisisResult = crisisDetectionService.detect(detectText, userId, "POST");
        if (crisisResult.isHit()) {
            if (crisisResult.isBlocked()) {
                AjaxResult blockedResult = AjaxResult.error("CRISIS_BLOCK");
                blockedResult.put("crisisLevel", crisisResult.getLevel());
                blockedResult.put("templateId", crisisResult.getTemplateId());
    ...
    public AjaxResult addPosts(Posts posts, HttpServletRequest request)
    {
        ...
        AjaxResult saveResult = addSave(posts);
        // 检查是否被危机拦截
        if (saveResult.get("msg") != null && "CRISIS_BLOCK".equals(saveResult.get("msg"))) {
            return saveResult;
        }
