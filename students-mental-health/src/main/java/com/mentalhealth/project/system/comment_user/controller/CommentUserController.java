...
public class CommentUserController extends BaseController
{
    ...
    public AjaxResult addForegroundComment(Comment comment) {
        ...
        // ===== 危机识别检测 =====
        CrisisDetectionResult crisisResult = crisisDetectionService.detect(
                comment.getContent(), user_id, "COMMENT");
        if (crisisResult.isHit()) {
            AjaxResult result = crisisResult.isBlocked()
                    ? AjaxResult.error("CRISIS_BLOCK")
                    : AjaxResult.success("CRISIS_WARN");
            result.put("crisisLevel", crisisResult.getLevel());
            result.put("templateId", crisisResult.getTemplateId());
            if (crisisResult.isBlocked()) {
                return result; // 高危：拦截，不落库
            }
            // 中危：继续落库，但前端要弹窗提示
            // 先落库再走，下面的逻辑不变
        }
